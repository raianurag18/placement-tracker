...
const ExperienceDetailPage = () => {
    ...
    useEffect(() => {
        const fetchExperience = async () => {
            try {
                const res = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/experiences/${id}`);
                const data = await res.json();

                if (res.ok) {
                    setExperience(data);
                } else {
                    setError(data.message || 'Failed to load experience');
                }
            } catch (err) {
                console.error(err);
            ...
            <main className="flex-1 p-8 lg:p-12 overflow-y-auto">
                <div className="max-w-5xl mx-auto space-y-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        {/* Main Content: Rounds & Experience */}
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="relative border-l-2 border-slate-200 ml-3 space-y-8 pl-8 pb-4">
                                    ...
                                    {(!experience.rounds || experience.rounds.length === 0) && (
                                        <div className="text-slate-400 italic">No detailed rounds info available for this experience.</div>
                                    )}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <Card className="border-slate-200 shadow-sm">
                                <CardContent className="p-6">
                                    ...
                                    <p className="text-sm text-slate-500 mb-4">Share this experience with your batchmates to help them prepare.</p>
